...
<div class="pafw-card pafw-<?php echo esc_attr( $master_id ); ?>">
	<?php if ( empty( $bill_key ) ) : ?>
        <div class="pafw-not-registered custom-handler" data-payment_method="<?php echo esc_attr( $payment_gateway->id ); ?>"></div>
	<?php else: ?>
        <div class="pafw-registered">
			...
			// translators:%s: card company name
    ...
    <div class="pafw_card_form" style="display: none;">
        <form>
            ...
            <div class="settlepg-register-card pafw-register-custom-handler" data-payment_method="<?php echo esc_attr( $payment_gateway->id ); ?>"><?php esc_html_e( "등록하기", "pgall-for-woocommerce" ); ?></div>
        </form>
    </div>
</div>
