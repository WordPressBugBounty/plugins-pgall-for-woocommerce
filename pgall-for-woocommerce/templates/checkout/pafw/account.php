...
<div class="pafw-billing-info" style="<?php echo is_user_logged_in() && 'yes' != $params[ 'show_account' ] ? 'display: none;' : ''; ?>">
	<?php if ( $has_billing_info ) : ?>
        <div class="pafw-account">
            <div class="customer_info">
				...
				<?php endif; ?>
                <p class="pafw-billing-name mshop-enable-kr mshop-always-kr"><?php printf( '%s%s ( %s, %s )', esc_html( $user->get_billing_first_name() ), esc_html( $user->get_billing_last_name() ), esc_html( $user->get_billing_email() ), esc_html( $user->get_billing_phone() ) ); ?></p>
				<?php if ( 'yes' == $params[ 'need_shipping' ] ) : ?>
                    <p class="pafw-billing-address mshop-enable-kr mshop-always-kr"><?php printf( '(%s) %s %s', esc_html( $user->get_billing_postcode() ), esc_html( $user->get_billing_address_1() ), esc_html( $user->get_billing_address_2() ) ); ?></p>
				<?php endif; ?>
            </div>
            <div class="edit">
	...
	<?php else: ?>
        <div class="pafw-account editable" style="cursor: pointer;">
            <div class="customer_info">
                <p class="customer_info_header"><?php esc_html_e( '등록된 고객정보가 없습니다. 고객정보를 등록해주세요.', 'pgall-for-woocommerce' ); ?></p>
            </div>
            <div class="edit">
                <div class="pafw-change-billing-info"></div>
            </div>
