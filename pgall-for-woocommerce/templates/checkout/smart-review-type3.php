...
<style>
    ...
    .pafw-review-wrapper > input[type=checkbox]:checked:before {
        ...
        width: 9px;
        height: 9px;
        background: url("<?php echo esc_url( PAFW()->plugin_url() . '/assets/images/rating/white/check.png' ); ?>") center center !important;
		...
		<?php foreach ( $rate_options as $rate_option ) : ?>
            <div class="pafw-smart-reivew-item">
                ...
                <label for="pafw_smart_review_rate<?php echo esc_attr( $rate_option[ 'rate' ] ); ?>">
                    <img src="<?php echo esc_url( PAFW()->plugin_url() . '/assets/images/rating/white/' . $rate_option[ 'rate' ] . '.png' ); ?>" style="height: 15px; vertical-align: middle;"/>
                    <span style="padding-left: 6px; vertical-align: middle;"><?php echo esc_html( $rate_option[ 'label' ] ); ?></span>
                </label>
            </div>
		<?php endforeach; ?>
        <div style="margin-top: 16px; <?php echo 'no' == get_option( 'pafw-user-can-edit-comment', 'no' ) ? 'display:none' : ''; ?>">
            <textarea name="pafw_smart_review_content" placeholder="<?php echo esc_html( get_option( "pafw-smart-review-placeholder", __( '리뷰를 작성 해 주세요.', 'pgall-for-woocommerce' ) ) ); ?>"></textarea>
        </div>
    </div>
</div>