...
if ( class_exists( 'WC_Payment_Gateway' ) ) {
	...
	class WC_Gateway_Payco extends PAFW_Payment_Gateway { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
		...
		public function add_approval_request_params( $params, $order ) {
			$params[ $this->get_master_id() ] = array(
				'reserved_order_no'          => pafw_get_unslash( $_REQUEST, 'reserveOrderNo' ),
				'payment_certify_token'      => pafw_get_unslash( $_REQUEST, 'paymentCertifyToken' ),
				'seller_order_reference_key' => pafw_get_unslash( $_REQUEST, 'sellerOrderReferenceKey' ),
			);
		...
		public function process_approval_response( $order, $response ) {
			...
			} elseif ( self::PAY_METHOD_VBANK == $response[ 'payment_method' ] ) {
			...
			} elseif ( self::PAY_METHOD_PHONE == $response[ 'payment_method' ] ) {
				$order->update_meta_data( "_pafw_hpp_num", $response[ 'hpp_num' ] );
			} elseif ( self::PAY_METHOD_SIMPLE_BANK == $response[ 'payment_method' ] ) {
		...
		function do_deposit_noti() {
			try {
				...
				$response = json_decode( pafw_get_unslash( $_REQUEST, 'response' ), true );
				...
				// translators: 1: error code, 2: erro message
		...
		function escrow_register_delivery_info() {
			...
			$this->add_log( 'escrow_register_delivery_info' . print_r( $_REQUEST, true ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
